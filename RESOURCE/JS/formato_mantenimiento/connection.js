...
searchContainers.forEach(container => {
    ...
    // La configuración básica solo necesita el input y el nombre del parámetro
    if (!paramName || !inputField) {
        console.error('Falta configuración en un contenedor de búsqueda (input o data-param-name).', container);
        return;
    }

    // --- LÓGICA DEL BOTÓN DE BÚSQUEDA (OPCIONAL) ---
    const searchButton = container.querySelector('.search-button');
    // Si el botón existe en este contenedor, le añadimos su funcionalidad
    if (searchButton) {
    ...
    // Si el contenedor es el de 'serie', configuramos las sugerencias
    if (paramName === 'serie') {
